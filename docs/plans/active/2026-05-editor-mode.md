...
CardioSimulator's Editor mode is currently a read-only multi-lead viewer with part-selection highlighting. RP5EcgEditor (the Delphi predecessor at `C:\eidosProject\trunk\RP5EcgEditor`) is a full waveform editor: point-level segment editing, block-level series composition, derived-lead generation, and DB persistence. This plan closes the functional gap on Android using the existing ZIP-backed data flow, so CardioSimulator becomes a usable replacement and not just a viewer.

## Current state

- Display side of the RP5 data model is already ported: `WaveformPart` / `EcgSeries` / `SourceSpec` / `AnchorPoint` / `EasingCurve` / `SeriesPartRef` all parse correctly in [`EcgData.kt`](../../../app/src/main/java/com/example/cardiosimulator/domain/EcgData.kt).
...
- Block flags (`FREQUENTLY`, `WITHSOUND`, `DURATIONFIXED`, `BROKEN`, `BEAT`, `SKIPQRS`) used heavily by RP5's Blocks tab are **not** represented anywhere in the Android domain model.
...
- Porting RP5's PostgreSQL layer. Storage stays ZIP/folder-based.
...
- Save-confirm dialog (Yes / No / Cancel) on screen-exit and operating-mode change — matches RP5's `SaveChanges`.
- Undo stack per record (mirror RP5's `FStackSourcePoints`).
...
### Phase 0a — Calibration parity with RP5

Prerequisite to anchor-driven editing. Today CardioSimulator ignores three fields it already parses — `SourceSpec.max`, `SourceSpec.value` ([EcgData.kt:42-43](../../../app/src/main/java/com/example/cardiosimulator/domain/EcgData.kt:42)) and `WaveformPart.duration` ([EcgData.kt:79](../../../app/src/main/java/com/example/cardiosimulator/domain/EcgData.kt:79)) — and renders every record through global constants `adcCountsPerMv = 256` and `sampleRateHz = 500` ([EcgCalibration.kt:8](../../../app/src/main/java/com/example/cardiosimulator/data/EcgCalibration.kt:8)). RP5 instead uses **per-record `AMax / AValue`** for both the grid (`AMax/AValue/10` px per mm at [Frame.Segments.pas:1142](../../../../../../eidosProject/trunk/RP5EcgEditor/Frames/Frame.Segments.pas:1142)) and the px↔mV/ms conversion (`P.Y / AMax * AValue` at [Frame.Segments.pas:522](../../../../../../eidosProject/trunk/RP5EcgEditor/Frames/Frame.Segments.pas:522), `CalcDuration(AMax, AValue, X)` at :526). Records whose `value`, `max`, or true sample rate diverge from CS's hard-coded defaults will misrender — wrong gain and/or wrong duration. Comparison memo: produced 2026-05-12 in chat.

- Promote `max` and `value` from `SourceSpec` onto `WaveformPart` (fall back to 200/2 when absent — matches RP5's defaults). Same for `EcgSeries` so series-level scale can read from its parts.
...
- **Editor-mode grid**: switch `Monitor`'s `pxPerMm` for Editor mode only to a *source-anchored* value `AMax/AValue/10` of the currently-edited record (matches RP5: one grid square = one source mm, so dragging an anchor moves an exact integer source-unit). Viewer modes (Teaching / Testing / Examination / OSKE) keep the physical-mm grid — they're displaying, not editing.
- **Series-view shrink**: when rendering a series (block timeline in Phase 2) apply RP5's hard-coded `ARatio = 2` halving against the first part's `AMax/AValue` ([Frame.Blocks.pas:1518](../../../../../../eidosProject/trunk/RP5EcgEditor/Frames/Frame.Blocks.pas:1518)). Implement the scaffolding here; the timeline consumer arrives in Phase 2.
...
**Verification:** load a fixture with non-default `max:400 value:5`, confirm the rendered amplitude matches RP5's screen capture of the same record to within 1 px. Load a fixture with `duration` implying 250 Hz, confirm the rendered duration matches RP5. Round-trip a record through save → reload → render and assert visual identity.

**Risk:** the per-part `amplitude` field used by [`EcgRepository.assembleWaveform`](../../../app/src/main/java/com/example/cardiosimulator/data/EcgRepository.kt:63) was baked by RP5's `GetPointsDevices` against the record's own `AMax/AValue` at export time, so mixing the new per-part scaling with the existing `amplitude` multiplication will double-scale unless `amplitude` is renormalized or treated as a pre-applied factor and removed from the renderer math. Decide which is authoritative (`amplitude` vs `max/value`) before merging — likely *re-derive* `amplitude` on save so it stays a function of `max/value` and the sample range.

### Phase 1 — Segment / point editor

Feature parity with RP5's Segments tab ([Frame.Segments.pas](../../../../../../eidosProject/trunk/RP5EcgEditor/Frames/Frame.Segments.pas)).

- Re-implement [`EditableLead`](../../../app/src/main/java/com/example/cardiosimulator/ui/display/EditableLead.kt) to render from `anchors` (with Bezier handling for `curve = b` triples) rather than baked `samples`. Cache rendered samples for speed; invalidate on edit.
...
- Move-all-anchors when dragging the first (origin) anchor — matches `MovePointsBy`.
...
**Risk:** Bezier-pair handling is subtle — RP5 treats a `b` flag as a 3-point group consumed together ([Frame.Segments.pas:1107](../../../../../../eidosProject/trunk/RP5EcgEditor/Frames/Frame.Segments.pas:1107)). Mirror exactly or the renderer will desync.

### Phase 3 — Derived-lead generation

Pure math, ported from [Frame.Segments.pas:690](../../../../../../eidosProject/trunk/RP5EcgEditor/Frames/Frame.Segments.pas:690) and :783. Sequenced ahead of Phase 2 because it's small and validates the Phase 0 serializer.
...
- "Generate derived leads" button on both the Segment editor (lead-level) and the Series editor (series-level — mirrors `Button17Click` at [Frame.Blocks.pas:1736](../../../../../../eidosProject/trunk/RP5EcgEditor/Frames/Frame.Blocks.pas:1736)).

### Phase 2 — Series / block timeline editor

Feature parity with RP5's Blocks tab ([Frame.Blocks.pas](../../../../../../eidosProject/trunk/RP5EcgEditor/Frames/Frame.Blocks.pas)).
...
- Drag a block left/right to reorder; drag edge handles to resize. Snap to neighbours — matches RP5's `AlignBlocks`.
...
Lower priority; raises perceived quality.

- HR=60 footer preview pane re-rendering the current segment/series in a loop (port `RecalcTumbPoints` / `RecalcTumbBlocks` sensor-window logic).
...
- **Bezier-group invariants.** RP5 silently assumes `b` curves come in triples. The Kotlin parser at [EcgData.kt:209](../../../app/src/main/java/com/example/cardiosimulator/domain/EcgData.kt:209) doesn't enforce this. Decide whether to validate on load or only on render. *(Open.)*
- **Touch precision vs. RP5's 1-px arrow nudging.** Compensating with pinch-zoom + numeric nudge panel. Verify on-device before Phase 1 lands. *(Open.)*
...
- **Block-flag wire format.** RP5 stores them in `TPointG.Flags`. The Android parser drops them today — need to decide the serialized representation (extra column inside the `(x,y,identy,offset)` tuple, or a separate `flags:` field). *(Open — resolve in Phase 2.)*
...
- **Phase 0a:** load fixtures with non-default `max`/`value` (e.g. 400/5) and non-500 Hz `duration`; visually compare against RP5 screen captures of the same record; assert pixel-accurate match for the 1 mV calibration pulse and full-scale amplitude.
...
- **Phase 3:** generate III from I+II on a known sample, compare numerically against the RP5 output for the same input.
